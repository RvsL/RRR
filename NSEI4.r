...
ohlc.within <- function(tdt, ldate1, ldate2)
{
t.1 <- tdt %>% filter(dt >= ldate1 & dt <= ldate2)
if(nrow(t.1) > 0){#open high low close
t.1.range <- range(t.1$navg)
t.1.last <- t.1$navg[length(t.1$navg)]
return(c(t.1$navg[1],t.1.range[2],t.1.range[1],t.1.last))} else {return(c(0,0,0,0))}
}

p2_q4_5 <- function(tdt, curdate)
{
ohlc.1 <- ohlc.within(tdt, curdate + 3600 * 9.25, curdate + 3600 * 12.5)
ohlc.2 <- ohlc.within(tdt, curdate + 3600 * 9.5, curdate + 3600 * 12.5)
ohlc.3 <- ohlc.within(tdt, curdate + 3600 * 9.5, curdate + 3600 * 13.5)
ohlc.4 <- ohlc.within(tdt, curdate + 3600 * 12.5, curdate + 3600 * 15.5)
ohlc.5 <- ohlc.within(tdt, curdate + 3600 * 13.5, curdate + 3600 * 15.5)
ohlc.6 <- ohlc.within(tdt, curdate + 3600 * 14.5, curdate + 3600 * 15.5)
a4 <- c(abs(ohlc.1[2] - ohlc.1[3]),abs(ohlc.2[2] - ohlc.2[3]),abs(ohlc.3[2] - ohlc.3[3]),abs(ohlc.4[2] - ohlc.4[3]),abs(ohlc.5[2] - ohlc.5[3]),abs(ohlc.6[2] - ohlc.6[3]))#high - low difference
a5 <- c(abs(ohlc.1[1] - ohlc.1[4]),abs(ohlc.2[1] - ohlc.2[4]),abs(ohlc.3[1] - ohlc.3[4]),abs(ohlc.4[1] - ohlc.4[4]),abs(ohlc.5[1] - ohlc.5[4]),abs(ohlc.6[1] - ohlc.6[4]))#start - end difference
return(rbind(a4, a5))
}
...
if(curdate > floor_date(begdate,"day")){
...
sma <- na.omit(SMA(tdt.xts$NIFTY.Adjusted, n = 55))
max.i <- nrow(sma)
sma.start <- as.numeric(sma[1])

if(tdt.range[2] == tdt.end.val & tdt$navg[1] <= sma.start){
i <- 2
while(as.numeric(sma[i-1]) <= as.numeric(sma[i]) & i < max.i){i <- i + 1}
p2.q6 <- abs(as.numeric(sma[i]) - tdt$navg[1])}

if(tdt.range[1] == tdt.end.val & tdt$navg[1] >= sma.start){
i <- 2
while(as.numeric(sma[i-1]) >= as.numeric(sma[i]) & i < max.i){i <- i + 1}
p2.q7 <- abs(as.numeric(sma[i]) - tdt$navg[1])}
}
...
coef.sign.15.01 <- trend.coef.before(tdt, curdate + 11.5 * 3600)
...
q.15.z11 <- ifelse(coef.sign.15.01 != coef.sign & q.15.b == 0 & q.15.c == 0 & q.15.a == 0 & q.15.z == 0, 1, 0)
...
p2.q4.5 <- p2_q4_5(tdt, curdate)

d <- cbind.data.frame(tdt.sd, curdate, tdt.start.val, 
tdt.end.val, tdt.range[1], tdt.range[2], 
tdt.volatility, q.13, q.14, q.15.a, q.15.b, q.15.c, q.15.z, q.15.z11,
...
q.20.45,
q.20.60,atr1,atr2,
q.9.vol,q.9.dt,q.18, q.18.r,q.10,q.16,q.11,q.12,tdt.move.down,tdt.mean,tdt.move.up,
p2.q4.5[1,1],
p2.q4.5[1,2],
p2.q4.5[1,3],
p2.q4.5[1,4],
...
p2.q4.5[2,5],
p2.q4.5[2,6],
p2.q6,p2.q7) %>% setNames(nm = c("sd","day", 
"start.val", "end.val", "range.min", "range.max", 
"volatility", "q.13", "q.14", "q.15.a", "q.15.b","q.15.c", "q.15.z", "q.15.z11", 
"q.17.a", "q.17.b", "q.17.c", "tdt.ups", 
...
res <- res %>% mutate(range.q2 = (start.val - end.val), abs.range.q2 = abs(range.q2), range.max.min = (range.max - range.min), abs.range.max.min = abs(range.max.min))
...
cat("part 2 Q1.1", "\n")
cat(summary(res$abs.range.max.min))

cat("part 2 Q1.2", "\n")
cat(summary(res$abs.range.q2))

cat("part 2 q1.1 sd = ",sd(res$abs.range.max.min),"q1.2 sd = ",sd(res$abs.range.q2),"\n")

cat("part 2 Q1.3.1", "\n")
for(i in 1:40){
t <- res %>% filter(abs.range.max.min > 5 * i)
cat(5 * i, " points = ", 100 * nrow(t)/nrow(res), "%\n")
}
...
cat("Q15 (<11:30)", sum(res$q.15.z11), "d.","\n")
...
res.p2 <- res %>% filter(p2.q6 != 0)
cat("p2.q6 ndays = ", nrow(res.p2)," avg pts = ", mean(res.p2$p2.q6),"\n")

res.p2 <- res %>% filter(p2.q7 != 0)
cat("p2.q7 ndays = ", nrow(res.p2)," avg pts = ", mean(res.p2$p2.q7),"\n")





#cd.1 <- as.POSIXct("2015-09-22")
#cd.2 <- as.POSIXct("2015-09-24")
#tdt.1 <- dt.examined %>% filter(dt >= cd.1 & dt < cd.2) %>% arrange(dt)
 
#tdt.1 <- xts(tdt.1[,-1], order.by=tdt.1[,1])
#plot.xts(tdt.1)